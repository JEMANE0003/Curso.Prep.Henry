...
function promedioResultadosTest(resultadosTest) {
  ...
  // Tu código:
  const sumaElementos = resultadosTest.reduce((acumulador, valor) => acumulador + valor);
...
function multiplicarArgumentos() {
  ...
  return argumentsArray.reduce((acumulador, valor) => acumulador * valor);
}
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  const numeroComoTexto = n.toString();
  return numeroComoTexto[0] === '9' ? true : false
}
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  const esIgual = arreglo.reduce((acumulador, valor) => acumulador === valor ? valor : false);
  return esIgual ? true : false
}
...
function breakStatement(numero) {
  ...
  return mensajeError ? mensajeError : array;
}
